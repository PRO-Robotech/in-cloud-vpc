...
│   Network Controller упал? → Текущая конфигурация сохраняется     │
...
│                                                                           │
│  ┌──────────┐  ┌──────────┐  ┌──────────────────┐  ┌─────────────────┐  │
│  │   API    │  │   IPAM   │  │ Network Controller│  │ Route Reflector │  │
│  └──────────┘  └──────────┘  └──────────────────┘  └─────────────────┘  │
│                                                                           │
├───────────────────────────────────────────────────────────────────────────┤
│                                                                           │
...
flowchart TB
    ...
    API --> IPAM
    API -.->|watch| NetCtrl
    NetCtrl -->|"config push"| Agent1
    NetCtrl -->|"config push"| Agent2
    NetCtrl -->|"config push"| Agent3
...
| **Config distribution** | Network Controller | HV не знает desired state нового NI | Только при изменении ресурсов |
| **Resource CRUD** | API | Точка входа для пользователя | Только при управлении ресурсами |
...
### 7.1 Принцип «push config, run independently»

Control-plane работает по модели **push + converge**:
...
sequenceDiagram
    ...
    participant Agent
    participant HV as "HV (Linux + FRR)"

    User->>API: Создать NI
    API->>IPAM: Allocate IP
    IPAM-->>API: IP assigned
    API-->>Controller: watch event
    Controller->>Agent: Push desired state
    Agent->>HV: Apply config (veth, bridge, FRR)
    Agent-->>Controller: Status OK

    Note over HV: С этого момента HV автономен.
    Note over HV: Controller может упасть —
    Note over HV: трафик продолжает идти.

    Controller-xAgent: Controller disconnected
    Note over HV: VM₁ ↔ VM₂ работают.
    Note over HV: Маршруты в FRR кешированы.
    Note over HV: VXLAN-туннели активны.
```

### 7.2 Agent: последний конфиг как источник истины на HV

Agent хранит последний полученный desired state локально (файл или embedded DB). Это позволяет:

| Сценарий | Поведение Agent |
|---|---|
| HV перезагрузился, Controller доступен | Agent получает fresh desired state, применяет |
| HV перезагрузился, Controller недоступен | Agent применяет последний сохранённый desired state |
| Controller пушит обновление | Agent вычисляет diff (desired ↔ actual), применяет delta |
| Controller недоступен долго | Agent периодически пытается reconnect; текущее состояние стабильно |
...
flowchart TB
    ...
    FD_CP -.->|"config push\n(не data path)"| FD_HV1
    FD_CP -.->|"config push"| FD_HV2
    FD_CP -.->|"config push"| FD_HV3
...
| **in-cloud (наша модель)** | Controller / IPAM / API | **Нет влияния.** FRR + kernel routing автономны |
...
sequenceDiagram
    ...
    API->>DB: UPDATE NI status → available
    API-->>Controller: watch: NI available

    Controller->>Controller: compute desired state for HV
    Controller->>Agent: gRPC: push config
    ...
    Agent-->>Controller: status: NI applied
    Controller->>API: update NI status
```

После последнего шага конфигурации — HV работает полностью автономно для данного NI. Control-plane можно выключить, и VM продолжат работать.
...
```mermaid
stateDiagram-v2
    [*] --> ConfigReceived: Controller push
    ConfigReceived --> Creating: Agent начинает apply
...
| NI configuration push | Controller → Agent | **Да** | Нет |
| Resource CRUD | API | **Да** | Нет |
...
**Итог:** гипервизор — это автономный маршрутизатор. Агрегированный слой (API, IPAM, Controller, RR) нужен для управления и оптимизации, но не для работы data-plane. Эта архитектура обеспечивает устойчивость: даже при полном outage control-plane существующие VM продолжают работать и общаться друг с другом.
